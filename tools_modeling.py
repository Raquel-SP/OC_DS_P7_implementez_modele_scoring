...
from sklearn.metrics import make_scorer, recall_score, fbeta_score, precision_score, accuracy_score, roc_auc_score, average_precision_score
from sklearn.metrics import confusion_matrix, ConfusionMatrixDisplay, RocCurveDisplay, PrecisionRecallDisplay
from sklearn.model_selection import cross_validate, RandomizedSearchCV, GridSearchCV, learning_curve  # , cross_val_score
...
def prob_threshold(model, X_val, y_val, title, df, score_fonction = custom_score, n=1, label=None, color='b', ax=None):
    '''
    ...
    score_fonction : score for which we want to evaluate the probability threshold
    ...
    for threshold in thresholds:
        ...
        sav_gains.append(score_fonction(y_val, y_pred))
    
    df = pd.DataFrame({'Thresholds' : thresholds,
                       'Bussines Score' : sav_gains})
    
    # Maximum business metric score
    gain_max = df['Bussines Score'].max()
    print(f'Maximum business metric score : {gain_max}')
    # Optimal threshold for the business metric
    threshold_max = df.loc[df['Bussines Score'].argmax(), 'Thresholds']
    ...
    if label is None:
        label = f'{score_fonction.__name__}'
...
class balance_method_analysis:
    ...
    def compute_metrics (self):
        ...
        # Bussines metric
        bussines_metric = custom_score(self.y_val, self.y_pred)
